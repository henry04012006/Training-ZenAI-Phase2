...
* `EPOCHS = 30`: High training epoch budget to allow the U-Net to fully converge.
* `LR = 2e-4`: Standard stable base learning rate, modulated via a `CosineAnnealingLR` decay schedule down to `1e-5`.
* `OPTIMIZER`: upgraded to `AdamW` with decoupled weight decay (`1e-4`) to prevent overfitting.
...
3. **Residual Network (ResNet) Blocks with Conditioning Injection & Dropout**
   * **Implementation Specifications:**
     ...
     * **Non-linear Activation & Dropout Regularization:** Utilize `SiLU` (Swish) activation for smooth gradients. Integrate a `nn.Dropout(p=0.1)` regularization layer directly between the second activation function and the secondary convolution layer to improve generalization stability during backpropagation:
       $$h_{\text{out}} = \text{Conv2d}(\text{Dropout}(\text{SiLU}(\text{GroupNorm}(h_{\text{cond}}))))$$
...
4. **Self-Attention Block (FlashAttention)**
   * **Theoretical Background & FlashAttention Optimization:** To capture long-range spatial dependencies and non-local semantic context, the U-Net incorporates self-attention layers at the $16 \times 16$ and $4 \times 4$ resolutions. The traditional PyTorch `nn.MultiheadAttention` module is replaced with PyTorch's native `F.scaled_dot_product_attention`. This utilizes highly optimized GPU kernels (such as FlashAttention or memory-efficient attention) to drastically reduce memory usage and accelerate computing speed by computing the attention matrix in a tiled fashion.
   * **Implementation Blueprint:**
     * Project the input features $h \in \mathbb{R}^{B \times S \times C}$ to a combined Query, Key, and Value tensor using a single linear layer (`self.qkv_proj = nn.Linear(C, C * 3)`).
     * Split and reshape the QKV tensor to shape $[B, N, S, d]$ where $N$ is the number of heads and $d = C / N$ is the head dimension.
     * Apply native `F.scaled_dot_product_attention(q, k, v)`.
     * Project the output back via `self.out_proj = nn.Linear(C, C)`, transpose back to the spatial layout $[B, C, H, W]$, and add the residual connection.
...
* **Down-path (Contracting Stream) & Stacked Layers:** Receives the initial $32 \times 32$ single-channel tensors. It utilizes exactly 3 spatial reduction operations (parameterized via `Conv2d` with a stride of 2), interleaved with `layers_per_block` (defaulting to 2) stacked sequential `ResNetBlock` primitives at each resolution stage:
  * *Scale 1:* $32 \times 32$ spatial dimensions. Stacks `layers_per_block` ResNet blocks.
  * *Scale 2:* $16 \times 16$ spatial dimensions. Stacks `layers_per_block` ResNet blocks. **Self-Attention Requirement:** To capture non-local structural configurations and macro-digit semantics, a Multi-Head Self-Attention module is strictly integrated at the output boundary of this stage.
  * *Scale 3:* $8 \times 8$ spatial dimensions. Stacks `layers_per_block` ResNet blocks.
...
* **Up-path (Expanding Stream) & Dimension Matching Rules:**
  * Progressively scales latent representations back up through structural dimensions ($4 \times 4 \rightarrow 8 \times 8 \rightarrow 16 \times 16 \rightarrow 32 \times 32$) using learnable fractionally-strided convolution layers (`nn.ConvTranspose2d(..., kernel_size=4, stride=2, padding=1)`).
  * **Skip Connections Concat Rule:** At each expansion step, incoming upsampled feature arrays are structurally concatenated with their exact spatial counterpart from the contracting down-path stream via `torch.cat([up_features, down_features], dim=1)`.
  * **Channel Tracking & Stacked Up-Layers:** Because concatenation doubles the incoming channel size, the subsequent stage of `layers_per_block` ResNet blocks starts with an input channel size of `C_up_in + C_down_out`. For instance, if the up-projected maps have 256 channels and the skip connection provides 256 channels, the first ResNet block of the stage accepts 512 input channels and projects them back to 256 channels, followed by another ResNet block projecting 256 to 256.
...
5. **Exponential Moving Average (EMA) Weight Tracking**
   * **Theoretical Background:** An Exponential Moving Average (EMA) of the model parameters is maintained during training. Because the objective function is highly non-convex, parameter trajectories during optimization fluctuate. EMA averages parameter updates over time:
     $$\theta_{\text{EMA}}^{(t)} = \beta \cdot \theta_{\text{EMA}}^{(t-1)} + (1 - \beta) \cdot \theta_{\text{online}}^{(t)}$$
     where $\beta = 0.999$ is the decay factor. The averaged weights $\theta_{\text{EMA}}$ represent a smoother version of the optimization path, yielding significantly cleaner generated images and lower inference FID/MSE.
   * **Implementation Blueprint:** A custom PyTorch `EMA` helper class holds a shadow copy of trainable model parameters.
     * `update(model)`: Called after each optimizer step to update the shadow weights in-place.
     * `apply_shadow(model)`: Temporarily swaps the model's online weights with the EMA shadow weights before saving checkpoints or generating samples.
     * `restore(model)`: Swaps the online weights back so training can resume seamlessly.
...
* **Graph Setup:** Instantiates the structural `UNet` core with high capacity (`base_channels=96`) and the mathematical `Diffusion` scheduler independently, transfers both to the accelerated target computing `device`, binds the model parameters to a `torch.optim.AdamW` optimizer (decoupled weight decay `1e-4`), and creates a `CosineAnnealingLR` scheduler (decaying to `1e-5` at epoch 30).
* **The Iterative Optimization Loop:**
  ```python
  # Ensure models are on the proper execution device
  model = UNet(..., base_channels=96).to(device)
  ...
  for epoch in range(EPOCHS):
      for batch_images, batch_labels in dataloader:
          ...
          # 3. Propagate error matrices and adjust weights
          loss.backward()
          optimizer.step()
          ema.update(model)
      
      scheduler.step()  # Decay learning rate at the end of each epoch
  ```
...
- [x] Optimize U-Net capacity by setting `base_channels = 96` for high-fidelity MNIST generation.
